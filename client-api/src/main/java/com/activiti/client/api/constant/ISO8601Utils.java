...
/**
 * Created by jpascal on 29/03/2015.
 */
...
{
    public static String DATE_ISO_FORMAT = "yyyy-MM-dd'T'HH:mm:ss.SSSZ";
    ...
    /**
     * Format a date into 'yyyy-MM-ddThh:mm:ssZ' (GMT timezone, no milliseconds
     * precision)
     *
     ...
     * @throws IllegalArgumentException if the date is not in the appropriate
     *             format
     */
    public static Date parse(String date)
    {
        try
        {
            ...
            // if (!timezone.getID().equals(timezoneId)) { throw new
    ...
    private static void checkOffset(String value, int offset, char expected) throws IndexOutOfBoundsException
    {
        ...
        if (found != expected) { throw new IndexOutOfBoundsException("Expected '" + expected
                + "' character but found '" + found + "'"); }
    }
    ...
    {
        if (beginIndex < 0 || endIndex > value.length() || beginIndex > endIndex) { throw new NumberFormatException(
                value); }
        ...
        if (i < endIndex)
        {
            ...
            if (digit < 0) { throw new NumberFormatException("Invalid number: " + value); }
            result = -digit;
        }
        while (i < endIndex)
        {
